...
from .models import User, Program, ExamDate, CommissionMember, Applicant, Question, Score, StandardCommission
from .forms import LoginForm, ProgramForm, ExamDateForm, AssignCommissionForm, ApplicantForm, ScoreForm, UserForm, \
    StandardCommissionForm, EditScoreForm, QuestionForm, UploadApplicantsForm
...
def programs():
    ...
    form = ProgramForm()
    if form.validate_on_submit():
        program = Program(code=form.code.data, name=form.name.data)
...
def exam_dates():
    ...
    if form.validate_on_submit():
        ...
        # Автоматически назначаем стандартную комиссию
        program = Program.query.get(form.program_id.data)
        for member in program.standard_commission:
            commission_member = CommissionMember(
                exam_date_id=exam_date.id,
                user_id=member.user_id,
                role=member.role
            )
            db.session.add(commission_member)
        db.session.commit()

        flash('Дата экзамена добавлена со стандартной комиссией')
...
def applicants():
    ...
    form = ApplicantForm()
    upload_form = UploadApplicantsForm()  # Новая форма для загрузки
...
def applicant_scores(applicant_id):
    ...
    # Проверка что сегодня день экзамена
...
def view_protocol(applicant_id):
    ...
    # комиссия может только в день экзамена
    if current_user.role == 'commission' and exam_date.date.date() != date.today():
        abort(403)
...
def standard_commission():
    ...
    if form.validate_on_submit():
        # Проверяем, нет ли уже такого члена комиссии для этой программы
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                # Находим или создаем программу
                program = Program.query.filter_by(code=program_code).first()
                if not program:
                    program = Program(code=program_code, name=program_name)
                    db.session.add(program)
                    db.session.commit()

                # Находим или создаем дату экзамена
                exam_date = None
                if exam_date_str:
                    try:
                        ...
                        if not exam_date:
                            exam_date = ExamDate(date=exam_date_obj, program_id=program.id)
                            db.session.add(exam_date)
                            db.session.commit()
                    except ValueError:
